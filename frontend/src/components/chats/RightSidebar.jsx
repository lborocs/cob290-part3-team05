...
const RightSidebar = ({
    ...
    const [newGroupName, setNewGroupName] = useState(selectedChat?.chatTitle || "");
            ...
            {selectedChat?.chatType === "Private" ? (
                <>
                    ...
                    <h2 className="text-lg font-bold mt-2">
                        {selectedUser ? `${selectedUser.firstName} ${selectedUser.lastName}` : "Unknown"}
                    </h2>
                    <p className="text-sm text-gray-300">{selectedUser?.userType || "Role not available"}</p>
                    <p className="text-sm text-gray-300">{selectedUser?.userEmail || "Email not available"}</p>
                ...
                <>
                    ...
                    <div className="flex flex-col items-center mt-4">
                        ...
                        <div className="mt-3 flex items-center gap-2">
                            <h2 className="text-lg font-bold text-white">{selectedChat?.chatTitle || "Group"}</h2>
                            <FaPencilAlt className="cursor-pointer text-white text-sm" onClick={openRenameModal} />
                        </div>
                    ...
                    <div className="mt-10 w-full">
                        ...
                        <div className="mt-2 space-y-2">
                            {(showAllMembers ? members : members.slice(0, 3)).map((user) => {
                                ...
                                const canRemove = isCurrentUserAdmin && user.userID !== currentUserID && user.userID !== selectedChat?.creatorID;
                                    ...
                                    >
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-sm text-white">
                                                {user.firstName} {user.lastName} <span className="text-xs text-gray-300">({user.userType})</span>
                                            </span>
                                        </div>
