...
const CreateChatModal = ({ onCancel, onCreateChat, currentUserID }) => {
  ...
  const currentUser = availableUsers.find(user => user.userID === currentUserID) || 
  ...
  const handleSubmit = (e) => {
    ...
    const userIDs = [currentUserID, ...selectedUsers.map(option => option.value)];
    ...
    <div className="fixed inset-0 bg-black/40 flex justify-center items-center z-50">
      <div className="bg-white rounded-xl shadow-lg w-[28rem] p-6 relative">
        ...
        <form onSubmit={handleSubmit}>
          {/* Chat Title */}
          <div className="mb-4">
            <label className="text-sm font-medium text-gray-700 mb-2 block">Chat Title</label>
            <input
              type="text"
              value={chatTitle}
          ...
          {/* Chat Type */}
          <div className="mb-4">
            <label className="text-sm font-medium text-gray-700 mb-2 block">Chat Type</label>
          ...
          <div className="mb-4">
            <label className="text-sm font-medium text-gray-700 mb-2 block">
              Select Users
              {chatType === "Private" && <span className="text-xs text-gray-500 ml-2">(Select one user for private chat)</span>}
            </label>
            ...
            {chatType === "Private" && selectedUsers.length > 1 && (
              <p className="text-red-500 text-xs mt-1">Private chats can only have one recipient</p>
            )}
          </div>
          ...
          <div className="flex justify-end">
            <button
              ...
              disabled={!chatTitle || (chatType === "Private" ? selectedUsers.length !== 1 : selectedUsers.length < 1)}
            >
              Create Chat
            </button>
          </div>
