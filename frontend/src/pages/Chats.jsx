    ...
    return (
        <div className="flex h-screen w-full">
            ...
            <div className="flex-1 flex flex-col relative bg-[var(--color-highlight)]">
                    ...
                    <div className="flex flex-col w-full max-w-3xl mx-auto space-y-2">
                        {messages.map((msg, index) => {
                            return msg.senderUserID === 0 ? (
                                <div key={`system-${index}`} className="w-full text-center text-xs text-gray-400 my-2">
                                    {msg.messageText}
                                </div>
                            ) : (
                                <div
                                    ...
                                    className={`flex items-center ${msg.senderUserID === currentUserID ? "justify-end" : "justify-start"}`}
                                >
                                    ...
                                    {editingMessageID === msg.messageID ? (
                                        ...
                                        <div className={`flex flex-col max-w-[75%] ${msg.senderUserID === currentUserID ? "items-end" : "items-start"}`}>
                                            {/* Name and Timestamp */}
                                            <div className="text-xs text-gray-500 mb-1 flex items-center gap-1 flex-wrap">
                                                <span>{msg.senderUserID === currentUserID ? "You" : msg.senderName}</span>
                                                <span className="text-gray-400">{formatTimestamp(msg.timestamp)}</span>
                                                {msg.is_edited && (
                                                    <span className="italic text-gray-400 text-[11px]">Edited</span>
                                                )}
                                            </div>

                                            <div className={`relative w-full flex ${msg.senderUserID === currentUserID ? "justify-end" : "justify-start"} group`}>
                                                {/* Dot Menu */}
                                                {!msg.is_deleted && (
                                                    <div
                                                        className={`
              ...
              ${msg.senderUserID === currentUserID ? "left-[-50px]" : "right-[-50px]"}
            `}
                                                    >
                                                        <MessageOptions
          ...
          `}
                                                >
                                                    {msg.is_deleted ? (
                                                        <span
                                                            className={`italic ${msg.senderUserID === currentUserID ? "text-white/60" : "text-gray-400"
                                                                }`}
                                                        >
